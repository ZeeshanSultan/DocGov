...
| A version nobody reads is decoration | Every file DocGov persists declares a `version`, and every read checks it before anything trusts the contents. Four already carried `version: 1` and nothing ever read it back, so a future DocGov writing version 2 would have been silently misread by an older build. Newer than this build is refused outright — guessing at a governance file is how a migration destroys a repository. A file written before versions were enforced has none, is treated as 1, and must never be rejected: that is every repository that has already adopted DocGov. |
