   ...
   3 enforcement      thin: call the      judgement only,
       rings           CLI, interpret     always advisory
        ...
        └──────────────────┼───────────────────┘
                           ▼
                  bin/docgov + cli/
             dispatch · parse · output · commands
                   hook protocol (cli/hooks.js)
                           │
                           ▼
                        core/
         ┌─────────────────┼─────────────────┐
...
The binary itself is a dispatcher of ninety lines. Everything it used to hold lives in `cli/`:
`parse.js` and `output.js` for the two things every command needs, `context.js` for the single
repository snapshot they all read, `hooks.js` for the whole hook protocol, and
`commands/{adopt,author,verify,state}.js` grouped by what the user is doing rather than by
what the code is.
...
| A plan is grouped by what it asks of the reader | Two thousand actions in one list reads as "this tool wants to rewrite my repository", and nothing on the page tells the reader which tenth actually needs them. Grouping by action kind answered a question nobody had; the question is "do I have to decide anything about this". Three tiers: nothing moves, something moves and the classification was clear, and DocGov could not settle it. The tier is derived from data every action already carried, and it changes nothing about what `fix` executes. The trap was in the wording rather than the grouping — `migrate` runs every MOVE, including ones whose classification was unclear, so a "needs review" tier that implied `fix` would skip them would have told the reader something false about their own repository. It says instead that `fix` will move them and that what is uncertain is what the document is. |
| The failures worth checking are agreements between files | A hook that never fires, a skill registered under the wrong slash command, an advertised option nothing reads, a generated-but-committed rules file describing last year's mode: none of these is a bug in any one file, so no test of any one file catches them, and every one of them is silent. `doctor` checks the agreements instead — and found two live ones on its first run, including the rules file in this repository saying `solo` while the config said `open-source`. Its own suggested repair was the second bug: `setup --force` rewrites `config.yaml` from scratch and takes every registration and domain with it, so the fix was more destructive than the fault. `setup --rules` regenerates only the generated file, which is what makes a generated-but-committed artifact self-healable at all. Cutting 0.3.0 found the next one: the version check compared two manifests while a third, the marketplace listing, declared one in two more places and nothing compared it. Every declared version is compared now, wherever it is nested. |
| A module global is a dependency nothing declares | `bin/docgov` carried `flags` and `JSONOUT` as module-level state every one of its forty functions could read and no signature mentioned. That, more than its length, is what made it the one file a mechanical edit could not be trusted on: command names collided with local identifiers, so a rename by search-and-replace produced duplicate bindings rather than an error, and nothing said which functions depended on which flag. Commands take `flags` as an argument now, and output mode is configured once in `cli/output.js` so no command has to remember to check it — which is also what stops a stray `console.log` landing inside a JSON payload a skill is parsing. The split found one live bug immediately: `pluginRootOf` counted directories up from its *caller*, which was right only while the caller was `bin/docgov`. |
| A project may extend the taxonomy and may not redefine it | No taxonomy enumerates every project's needs: after all the classification work, three unfamiliar repositories still left working notes, sales collateral and QA evidence with nowhere to go, and the only escape hatch was abstention — correct, and terminal. Classes declared in config or a policy pack are merged into `TYPES` at config load, which is the one place every entry point passes through. Two constraints are what stop it being a hole. A custom class cannot take a shipped id, because redefining `product.prd` would make every other repository's answer to what that means unverifiable. And `paths` is mandatory: a class with no path evidence could only ever be guessed at from prose, which is exactly the confident wrongness the classifier was rebuilt to eliminate — so a document outside those paths still abstains rather than falling into the nearest class somebody invented. |
| Two reviews, because they are two questions | One prompt did audience fit and leak detection together, against one generic standard, and both halves of that were wrong. "Does this read like a README" is a quality judgement where a false positive costs an argument about prose somebody was right about; "might this leak a credential" is a security question where a false negative publishes a secret. Opposite tolerances cannot share a prompt, a severity or a switch. And no single standard judges every class: a README is measured on a stranger getting running in five minutes, a runbook on being executable at 3am by someone who did not write it. Told only to "review this document", a model applies whichever standard occurs to it, and the finding is unfalsifiable because the standard was never stated. The engine picks the lens — it knows the class — and hands over the lens document itself, so the author can argue with a written standard instead of with a model. Leak detection now also runs deterministically in the write hook, on edits as well as new files, because a credential is just as published when pasted into a document that already existed. |
| Duplicated text and duplicated responsibility are different problems | The near-copy check finds documents copied from one another; on a real repository nearly all of its hits were deliberate. Competing sources of truth do not read alike — they were written independently by people who did not know the other existed, which is why they compete. A cluster needs the same audience, a distinctive shared name word, corroborating body vocabulary, and no stated relationship or organising index. Both the name and the body are required, and that is measured rather than cautious: in a repository about one subject everything is about that subject, and on this project's own documentation CONTRIBUTING scores 0.283 against the testing strategy and the README 0.433 against the command reference, none of which compete for anything. The first attempt used transitive closure over "related to" and returned a single cluster of 900 files. The known cost is stated rather than papered over: two documents named entirely differently for one subject are not found, because catching them needs body similarity to decide alone and the numbers say it cannot. |
| Several agents in one repository is the normal case now | The migration safety story rests entirely on the plan describing the repository that is actually there, and with a main agent and subagents that stopped being a safe assumption: one writes documentation while another still holds a plan that predates it. A plan now records a fingerprint of the documents it was computed against — the commit too, because that is what a human recognises, but the documents are what matter, since an uncommitted edit changes the plan's assumptions and a commit touching no documentation does not. `fix` refuses on a mismatch and names what was added, removed or modified, because "run review again" with no reason is a message people learn to skip. Every write is atomic as well: `writeFileSync` truncates before it writes, so a concurrent reader could see half a registry, and a rename on the same filesystem cannot. And a plan from before this existed has no fingerprint and still runs. |
| A monorepo has more than one authority | A package's architecture document is canonical for that package and says nothing about any other, and every package has a README. Repository-wide authority gets both wrong, and the visible cost was destinations: on a 1,011-document monorepo, 22 package documents were proposed for the root tree, `chainsaw-landing/AUDIT_REPORT.md` among them. A scope is read from the boundaries the build tooling already draws — a directory with a package manifest — so nothing here invents one; `node_modules`, `vendor` and fixture trees are excluded, and the root is not a scope because it is what everything else is nested inside. Inside a scope: destinations stay in the package, singletons are per scope, and competing-responsibility comparisons do not cross packages. Honest limit: the plan barely shrinks — 345 moves became 343, because that repository's documents are mostly in `docs/`, `internal/` and `qa/`, which are not packages. Scoping fixes where package documents go, not how much documentation a large repository has. |
...
| Annotating a document rewrites only the lines DocGov owns | `patchDocgov` rebuilt the whole frontmatter block from the parsed object. Keys, values, types and ordering all round-tripped correctly, and every comment was silently deleted — the strict YAML subset parses to a plain object, and a comment has nowhere in an object to live. Preserving them looked like it needed a comment-aware YAML round-trip, which is a project rather than a patch. It needed the opposite: not re-serialising what was never changed. Only the `docgov:` block is DocGov's to write, so only those lines are replaced and everything else — comments, blank lines, quoting style, key order, a Hugo `weight` and the note explaining it — is carried across as text. What remains lost is a comment *inside* the docgov block, which is the one part of the file DocGov owns and regenerates. This is the same principle that refuses TOML and JSON frontmatter outright: a tool that edits people's files must not quietly delete what they wrote. |
